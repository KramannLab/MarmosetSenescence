...
htos %>% group_by(HTO.global) %>%
	...
	data.frame() %>%
	write.table(., 
		file = paste0(outdir, sample, '.HTO.classification.global.txt'), 
		sep = '\t', 
		quote = FALSE, 
		row.names = FALSE)
...
htos %>% group_by(HTO) %>%
	...
	data.frame() %>%
	write.table(., 
		file = paste0(outdir, sample, '.HTO.classification.txt'), 
		sep = '\t', 
		quote = FALSE, 
		row.names = FALSE)
